...
# Windows console mặc định dùng cp1252 → ép UTF-8 để in được tiếng Việt + emoji
...
# ragas 0.4.3 import cứng 'langchain_community.chat_models.vertexai' — module này
# đã bị gỡ khỏi langchain-community mới (1.x). Tạo stub để ragas import được.
# Lab dùng OpenAI nên KHÔNG thực sự cần ChatVertexAI; stub chỉ để qua bước import.
...
if "langchain_community.chat_models.vertexai" not in sys.modules:
    try:
        ...
        class ChatVertexAI:  # stub đủ để ragas import được symbol
            ...
...
# SYSTEM_V1 và SYSTEM_V2 — giống hệt file 02_prompt_hub_ab_routing.py để kết quả so sánh được
SYSTEM_V1 = (
    "Bạn là trợ lý AI thân thiện. Chỉ dùng context sau để trả lời câu hỏi. "
    "Giữ câu trả lời ngắn gọn và rõ ràng (2-4 câu). "
    "Nếu context không chứa thông tin cần thiết, hãy nói thẳng là bạn không biết.\n\n"
    "Context:\n{context}"
)
...
SYSTEM_V2 = (
    "Bạn là chuyên gia phân tích thông tin. Khi trả lời, hãy đọc kỹ context, "
    "xác định các facts liên quan, rồi viết câu trả lời có tổ chức (3-5 câu) gồm: "
    "(1) ý chính, (2) chi tiết hỗ trợ trích từ context, (3) mức độ chắc chắn. "
    "Chỉ dựa trên dữ liệu được cung cấp, không suy đoán thêm. "
    "Nếu context không đủ thông tin, nêu rõ điều đó.\n\n"
    "Context:\n{context}"
)
