...
class PIIDetector(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        # Lặp qua từng loại PII và pattern tương ứng
        ...
        if found_pii:
            ...
            # Có PII → FailResult + fix_value; OnFailAction.FIX sẽ thay output bằng text đã redact
            return FailResult(
...
class JSONFormatter(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        """
        # Thử parse JSON trực tiếp → đã hợp lệ thì pass
        try:
            json.loads(value)
            return PassResult()
        ...
        try:
            ...
            # Sửa được → FailResult + fix_value; OnFailAction.FIX thay output bằng JSON chuẩn
            return FailResult(
                error_message="JSON lỗi đã được sửa",
...
def demo_pii_guard():
    ...
    # Tạo Guard với PIIDetector, truyền on_fail vào CONSTRUCTOR của validator
...
def demo_json_guard():
    ...
    # Tạo Guard với JSONFormatter, truyền on_fail vào CONSTRUCTOR của validator
